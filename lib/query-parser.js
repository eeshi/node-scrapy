...
// up the content. In both cases above, if we do a Sting#replace, we endup with the string:
//   "whatever content here"
const leadingAndTrailingBundaries = /(^\s*\(\s*)|(\s*\)\s*$)/g
...
function parseQuery(query) {
  ...
  const queryExtensionContent = queryExtension.replace(leadingAndTrailingBundaries, '')
