...
function extractItem(dom, item, selectorEngine, getters, filters) {
  if (isArray(item)) {
    ...
    const matches = selectorEngine.selectAll(queryAST.selector, dom)

    if (!matches || !matches.length) return null

    if (isArray(item[1]) || isObject(item[1])) {
      return matches.map(
        (context) => extractItem(context, item[1], selectorEngine, getters, filters)
      )
    }
  ...
  if (isString(item)) {
    ...
    const match = selectorEngine.selectOne(query.selector, dom)
